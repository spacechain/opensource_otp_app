...
//  static final String SECRET_COLUMN = "secret";
    @VisibleForTesting
    static final String PUBKEY_COLUMN = "pubkey";
    @VisibleForTesting
    static final String PRIVKEY_COLUMN = "privkey";
    @VisibleForTesting
    ...
    @VisibleForTesting
    static final String TABLE_NAME = "spc_accounts";
...
//  }
    ...
    public static class AccountIndex implements Serializable {
         ...
         */
        public AccountIndex(String name, String issuer,String pubkey,String privkey ) {
...
//    }
    ...
    public SpcAccountDb(Context context) {
        ...
        mDatabase.execSQL(String.format(
                ...
                PUBKEY_COLUMN, // Required
                PRIVKEY_COLUMN, // Required
    ...
    public AccountIndex findSimilarExistingIndex(AccountIndex index) {
        ...
        try {
            ...
            int nameIndex = cursor.getColumnIndex(SpcAccountDb.NAME_COLUMN);
            int pubkeyIndex =  cursor.getColumnIndex(SpcAccountDb.PUBKEY_COLUMN);
            int privkeyIndex =  cursor.getColumnIndex(SpcAccountDb.PRIVKEY_COLUMN);
            while (cursor.moveToNext()) {
                AccountIndex cursorIndex = new AccountIndex(cursor.getString(nameIndex), index.getIssuer(),cursor.getString(pubkeyIndex),cursor.getString(privkeyIndex));
...
//    return null;
//  }
    public String getPubkey(AccountIndex index) {
        Cursor cursor = getAccountCursor(index);
        try {
            if (!cursorIsEmpty(cursor)) {
                cursor.moveToFirst();
                return cursor.getString(cursor.getColumnIndex(PUBKEY_COLUMN));
            }
        } finally {
            tryCloseCursor(cursor);
        }
    ...
    public String getPrivkey(AccountIndex index) {
        Cursor cursor = getAccountCursor(index);
        try {
            if (!cursorIsEmpty(cursor)) {
                cursor.moveToFirst();
                return cursor.getString(cursor.getColumnIndex(PRIVKEY_COLUMN));
            }
        } finally {
            tryCloseCursor(cursor);
        }
...
//        }
     ...
     */
    public AccountIndex add(String name, String privkey, String pubkey, Integer counter,
...
//    Preconditions.checkNotNull(secret);
        ...
        ContentValues values = newContentValuesWith(privkey, pubkey, counter, googleAccount);
        AccountIndex indexToAdd = new AccountIndex(name, issuer,pubkey,privkey);
        ...
        if ((issuer != null) || name.equals(GOOGLE_CORP_ACCOUNT_NAME)) {
            ...
            while (!insertNewAccount(values)) {
                ...
                indexToAdd = new AccountIndex(name + "(" + tries + ")", issuer,pubkey,privkey);
    ...
    private ContentValues newContentValuesWith(
            String privkey, String pubkey, Integer counter, Boolean googleAccount) {
        ContentValues values = new ContentValues();
        if (privkey != null) {
            values.put(PRIVKEY_COLUMN, privkey);
        }
        if (pubkey != null) {
            values.put(PUBKEY_COLUMN, pubkey);
        }
        if (counter != null) {
    ...
    public boolean update(
            AccountIndex index, String privkey, String pubkey, Integer counter, Boolean googleAccount) {
        Preconditions.checkNotNull(index);
    ...
    private boolean insertNewAccount(ContentValues values) {
        ...
        AccountIndex index =
                new AccountIndex((String) values.get(NAME_COLUMN), (String) values.get(ISSUER_COLUMN),(String) values.get(PUBKEY_COLUMN),(String) values.get(PRIVKEY_COLUMN));
        if (indexExists(index)) {
    ...
    public List<AccountIndex> getAccounts() {
        ...
        try {
            ...
            int nameIndex = cursor.getColumnIndex(SpcAccountDb.NAME_COLUMN);
            int issuerIndex = cursor.getColumnIndex(SpcAccountDb.ISSUER_COLUMN);
            int pubkeyIndex = cursor.getColumnIndex(SpcAccountDb.PUBKEY_COLUMN);
            int privkeyIndex = cursor.getColumnIndex(SpcAccountDb.PRIVKEY_COLUMN);
            ...
            for (int i = 0; i < nameCount; ++i) {
                ...
                String privkey = cursor.getString(privkeyIndex);
                String issuer = null;
                if (issuerIndex >= 0) {
                    issuer = cursor.getString(issuerIndex);
                }
                resultBuilder.add(new AccountIndex(name, issuer,pubkey,privkey));
            }
